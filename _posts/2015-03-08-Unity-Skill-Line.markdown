...
从外部导入的动画一般是3d模型格式的。虽然不能编辑外部导入的动画，但可以进行一些调整。从Project窗口选中一个模型，然后在Inspector窗口切换到Animations选项卡，如果模型有附带动画，会在这里列出来。如下图，展示了一个模型附带了7个动画片段。在这里可以进行分割动画、调整动画开始和结束帧、预览动画等操作。
